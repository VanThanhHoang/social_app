...
const SignUpScreen = () => {
    ...
    const handleSignUp = async (data: any) => {
        ...
        try {
            const response: any = await registerUser(email, password, selectedGender, 1);
    ...
    return (
        ...
        <ScrollView nestedScrollEnabled={true} style={styles.container}>
            ...
            <TextInputSignIn style={[styles.input, errors.email ? styles.inputError : null]}{...register('email')}
                placeholder="Email"
                title="Email"
                showIcon
                iconType='email'
            ...
            <TextInputSignIn
                ...
                showIcon
                iconType='name'
                style={[styles.input, errors.fullName ? styles.inputError : null]}{...register("fullName")}
                onChangeText={(text: string) => {
                    setValue('fullName', text, { shouldValidate: true });
                }} />
            {errors.fullName && <Text style={styles.errorText}>{errors.fullName.message}</Text>}
            <Text style={{ color: colors.black, fontSize: 16, fontWeight: 'bold', marginTop: 15 }}>Gender</Text>
            <SelectList
                data={gender}
                placeholder="Select gender"
                boxStyles={{ width: 370, height: 60, backgroundColor: colors.greyLight, borderWidth: 0, marginTop: 10 }}
                inputStyles={{ marginTop: 10, marginStart: 20 }}
                arrowicon={<FontAwesomeIcon icon={faVenusMars} size={15} color="#000" style={styles.icon} />}
                search={false}
                dropdownStyles={{ width: 370, height: 130, backgroundColor: colors.greyLight, borderWidth: 0 }}
                setSelected={(item: any) => {
                    setValue('selectedGender', item, { shouldValidate: true });
                }}
            />
            {errors.selectedGender && <Text style={styles.errorText}>{errors.selectedGender.message}</Text>}
            <TextInputSignIn
                ...
                style={[styles.input, errors.password ? styles.inputError : null]} {...register('password')} />
            {errors.password && <Text style={styles.errorText}>{errors.password.message}</Text>}
            <TextInputSignIn
                placeholder="Confirm Password"
                title="Confirm Password"
                showIcon iconType='password'
                style={[styles.input, errors.confirmPassword ? styles.inputError : null]} {...register('password')}
                onChangeText={(text: string) => {
